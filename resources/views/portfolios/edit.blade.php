                ...
                <form id="portfolio-update-form" method="POST" action="{{ route('portfolios.update', $portfolio->id) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input
                            ...
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-300 dark:bg-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            placeholder="Voer de tweede titel in"
                        >
                    </div>
                    ...
                    @php
                        ...
                        $selectedColorIndex = old('color_index', $portfolio->color_index ?? 0);
                    @endphp
                        ...
                        <div class="flex flex-wrap gap-3">
                            @foreach($portfolioPalette as $index => $color)
                                <label class="cursor-pointer">
                                    <input type="radio" name="color_index" value="{{ $index }}" {{ $selectedColorIndex == $index ? 'checked' : '' }} class="hidden color-radio">
                                    <div class="w-12 h-12 rounded-lg border-2 transition-all {{ $selectedColorIndex == $index ? 'border-gray-800 dark:border-gray-200 ring-2 ring-blue-500' : 'border-gray-300 dark:border-gray-600' }}" style="background-color: {{ $color['bg'] }}; border-color: {{ $selectedColorIndex == $index ? '#1f2937' : $color['border'] }};" title="Kleur {{ $index + 1 }}"></div>
                                </label>
                            @endforeach
                        </div>
                    </div>
